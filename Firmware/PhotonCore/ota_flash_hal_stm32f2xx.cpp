...
int HAL_Set_System_Config(hal_system_config_t config_item, const void* data, unsigned data_length)
{
    ...
    case SYSTEM_CONFIG_SOFTAP_HOSTNAMES:
        break;
	case SYSTEM_CONFIG_SOFTAP_DISABLE_BROADCAST: //2WIRELESS
		const char* disable;
		disable = (const char*)data;
		if (strcmp(disable,"0") == 0) {
			disable_ssid_broadcast(false);
		} else {
			disable_ssid_broadcast(true);
		}
        break;
    }
